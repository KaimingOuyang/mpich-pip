...
int MPIDI_IPCI_send_lmt_ctrl_hdr_pkt_cb(MPIDI_SHMI_ctrl_hdr_t * ctrl_hdr)
{
    int mpi_errno = MPI_SUCCESS, mpl_err;
    MPIDI_IPC_ctrl_send_lmt_hdr_pkt_t *slmt_ctrl_hdr = &ctrl_hdr->ipc_slmt_hdr_pkt;
    MPIDU_shm_seg_t *memory = NULL;

    MPIR_FUNC_VERBOSE_STATE_DECL(MPID_STATE_MPIDI_IPCI_SEND_LMT_CTRL_HDR_PKT_CB);
    MPIR_FUNC_VERBOSE_ENTER(MPID_STATE_MPIDI_IPCI_SEND_LMT_CTRL_HDR_PKT_CB);
  ...
  fn_exit:
    MPL_shm_hnd_finalize(&(memory->hnd));
    MPL_free(memory);
    MPIR_FUNC_VERBOSE_EXIT(MPID_STATE_MPIDI_IPCI_SEND_LMT_CTRL_HDR_PKT_CB);
    return mpi_errno;
  fn_fail:
    goto fn_exit;
}

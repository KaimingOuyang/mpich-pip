...
{
    int extent;
    MPIR_Comm *comm = pcomm->node_comm;
    int local_rank = comm->rank;
    int local_size = comm->local_size;
    int mpi_errno = MPI_SUCCESS;
    void *root_buf;
    ...
    if (pcomm->node_procs_min == pcomm->node_comm->local_size) {
        goto fn_exit;
    }
    ...
    if (local_rank == root) {
        ...
        MPIR_PIP_Comm_opt_intra_barrier(comm, local_size);
        __sync_synchronize();
        while (sindex != eindex) {
            local_task = comm->tcoll_queue[round][sindex];
            if (local_task->free)
    ...
    } else if (comm->pip_roots_comm == NULL) {
        do {
            while (tcoll_queue_array[root][round][eindex] == NULL)
                MPL_sched_yield();
            local_task = tcoll_queue_array[root][round][eindex];

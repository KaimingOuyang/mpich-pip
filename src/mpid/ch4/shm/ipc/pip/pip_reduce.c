                                               ...
                                               MPIR_Errflag_t * errflag)
{
    ...
    MPIDI_PIP_Coll_easy_task_t *shared_addr, *rem_addr;
    int root_completee_step = 1, if_send, rem_proc, if_rem;
    ...
    while (pofk_1 <= rem_step_limit)
        pofk_1 *= basek_1;
    if_rem = (rem_step - pofk_1) % pofk_1;
    rem = rem_step - pofk_1;
    if_send = (rem % pofk_1 == 0) ? 0 : 1;
    rem_proc = rem / pofk_1;
    ...
    if (if_rem > 0) {
        ...
        mpi_errno =
            MPIDI_PIP_Reduce_recursive_bruck_internode(recvbuf, rem_buf, count, datatype, op,
                                                       comm, if_rem, errflag);
    }

    if (local_rank == 0) {
        shared_addr = MPIR_Comm_post_easy_task(recvbuf, TMPI_Allreduce, 0, 0, new_leader_num, comm);
    ...
    if (my_rem > 0) {
        ...
        if (my_rem == pofk_1) {
            // if (if_send && rem_proc > 0) {
            //     mpi_errno = MPIC_Send(&rem_buf, 1, MPI_LONG_LONG, rem_proc, 7,
            //                           comm->node_comm, errflag);
            //     MPIR_ERR_CHECK(mpi_errno);
            // }
            ...
            // mpi_errno =
            //     MPIC_Recv(&root_rem_buf, 1, MPI_LONG_LONG, 0, 7, comm->node_comm, MPI_STATUS_IGNORE,
            //               errflag);
            // MPIR_ERR_CHECK(mpi_errno);

            mpi_errno = MPIC_Sendrecv(rem_addr->addr, count, datatype,
                                      dst, MPIR_ALLREDUCE_TAG, tmp_buf, count, datatype,
                                      src, MPIR_ALLREDUCE_TAG, comm, MPI_STATUS_IGNORE, errflag);
            __sync_fetch_and_add(&rem_addr->complete, 1);
        }
        MPIR_ERR_CHECK(mpi_errno);

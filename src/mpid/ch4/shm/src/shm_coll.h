        ...
        const void *algo_parameters_container)
{
	...
	size_t data_sz = MPIR_Datatype_get_basic_size(datatype) * count;
	if (comm_ptr != NULL && data_sz >= 256) {
		if (comm_ptr->rank == 0) {
			if (root != 0) {
				rank0_tmp_buffer = MPL_malloc(data_sz, MPL_MEM_OTHER);
			} else {
		...
		// }
		// // if (comm_ptr->rank == 0) {
		// printf("First stage reduce rank %d, data_sz %ld BEGIN\n", comm_ptr->rank, data_sz);
		// fflush(stdout);
		// }

		ret = MPIDI_XPMEM_mpi_reduce(sendbuf, rank0_tmp_buffer, count, datatype, op, 0, comm_ptr, errflag, algo_parameters_container);
		if (ret != MPI_SUCCESS)
			goto fn_exit;

		// if (comm_ptr->rank == 0) {
		// 	printf("First stage reduce rank %d END\n", comm_ptr->rank);
		// 	fflush(stdout);
		// }

...
/* MPIDI_SHM_do_large_ctrl_send adopts POSIX shared memory to transfer large ctrl header. Caller should keep
 * the memory until receiving fin packet. */
MPL_STATIC_INLINE_PREFIX int MPIDI_SHM_do_large_ctrl_send(int rank, MPIR_Comm * comm,
                                                          ...
                                                          void *ctrl_hdr, MPIR_Request * req)
{
    ...
    lmt_ctrl_hdr =
        (MPIDI_SHMI_ctrl_hdr_t *) MPL_malloc(sizeof(MPIDI_IPC_ctrl_send_lmt_hdr_pkt_t) +
                                             serialized_hnd_size, MPL_MEM_OTHER);
    lmt_ctrl_hdr->ipc_slmt_hdr_pkt.ctrl_id = ctrl_id;
    lmt_ctrl_hdr->ipc_slmt_hdr_pkt.ctrl_hdr_size = ctrl_hdr_size;
    memcpy(lmt_ctrl_hdr->ipc_slmt_hdr_pkt.serialized_hnd, serialized_hnd, serialized_hnd_size);
    ...
    mpi_errno = MPIDI_POSIX_am_send_hdr(rank, comm, MPIDI_POSIX_AM_HDR_SHM,
                                        MPIDI_IPC_SEND_LMT_HDR_PKT, lmt_ctrl_hdr,
                                        sizeof(MPIDI_IPC_ctrl_send_lmt_hdr_pkt_t) +
                                        serialized_hnd_size);

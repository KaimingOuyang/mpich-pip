...
extern char tmp_buffer[2][8388608];
        ...
        * ch4_algo_parameters_container)
{
	...
	if (comm->node_roots_comm != NULL) {
		// printf("Allocate node_roots_comm memory\n");
		// fflush(stdout);
	...
	if (comm->node_comm != NULL && MPIR_Get_intranode_rank(comm, root) != -1) {
		// printf("Rank: %d, final step to merge results %d\n", comm->rank, *(int*)sendbuf);
		// fflush(stdout);
#ifndef MPIDI_CH4_DIRECT_NETMOD
		mpi_errno =

...
int MPIR_Reduce_intra_binomial(const void *sendbuf,
	...
	while (/*(mask & relrank) == 0 && */ mask < comm_size) {
		/* Receive */
		// printf("rank: %d MPIR_Reduce_intra_binomial while\n", rank);
		// fflush(stdout);
		if ((mask & relrank) == 0) {
			source = (relrank | mask);
			if (source < comm_size) {
				...
				// printf("rank: %d MPIR_Reduce_intra_binomial recv\n", rank);
	...
	if (!is_commutative && (root != 0)) {
		// printf("rank: %d MPIR_Reduce_intra_binomial !is_commutative\n", rank);
		// fflush(stdout);
		if (rank == 0) {
	...
	// printf("rank: %d MPIR_Reduce_intra_binomial end\n", rank);
	// fflush(stdout);
fn_exit:
